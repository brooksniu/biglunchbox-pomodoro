function getTasks() {
    ...
    if (localStorage.getItem('tasks') === null || localStorage.getItem('id') === null || localStorage.getItem('theme') === null || localStorage.getItem('volume') === null || localStorage.getItem('state') === null || localStorage.getItem('clickState') === null || localStorage.getItem('alarmState') === null) {
        tasks = [];
        id = 0;
        theme = 'light';
        volume = 50;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < tasks.length; i += 1) {
        ...
        if (tasks[i].checked) {
            ...
            task.shadowRoot.children[0].children[1].style.display = 'none';
        } else {
            task.style.display = 'flex';
            task.shadowRoot.children[0].children[1].style.display = 'initial';
        }
